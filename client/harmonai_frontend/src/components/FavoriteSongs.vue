<template>
    <div class="favorite-songs-section" id="favorite-songs" v-if="activeSection === 'favorite_songs'">
        ...
        <ul class="song-list-container">
            ...
            <li class="song-card" v-for="song in dummy_songs" :key="song.name">
            ...
            <li v-if="songs.length === 0" class="empty-song-list-message">
                <p>{{ $t('song.noFavoriteSongsFound') }} <a href="/">{{ $t('nav.home') }}</a></p>
            </li>
        </ul>
...
export default {
    ...
    data() {
        return {
            // dummy songs is a temporary list of songs as a placeholder to test how the UI looks
            dummy_songs: [
                {
            ...
            url: 'http://localhost:8001/api/songs',
    ...
    mounted() {
        ...
        // since the songs service is not online yet, we do not want to fetch the songs on page load because that just causes errors
        // this.getUserFavoriteSongs() // we get the user favorite songs as soon as the page is loaded
    },
    methods: {
